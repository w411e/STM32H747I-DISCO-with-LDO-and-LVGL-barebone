...
#define PWR_SUPPLY_CONFIG_MASK (PWR_CR3_LDOEN) // ADDED
...
void PLL_Config480Mhz(void)
{
	...
	if( ((RCC->CR) & RCC_CR_HSERDY) )
	{
		...
		MODIFY_REG (PWR->CR3, PWR_SUPPLY_CONFIG_MASK, PWR_CR3_LDOEN); // ADDED
		...
		while(__RCC_GET_FLAG(RCC_FLAG_PLLRDY) != 0U){} // Changed from != to == ADDED
		...
		MODIFY_REG(FLASH->ACR, FLASH_ACR_LATENCY, FLASH_ACR_LATENCY_2WS); // Changed from 2WS to 5WS
