...
#define PWR_REGULATOR_VOLTAGE_SCALE0  (PWR_D3CR_VOS_0 ) // ADDED only 480 MHz freq
#define PWR_SUPPLY_CONFIG_MASK (PWR_CR3_LDOEN | PWR_CR3_SMPSEN | PWR_CR3_BYPASS) // ADDED
...
#define __RCC_GET_FLAG(__FLAG__) (((((((__FLAG__) >> 5U) == 1U)? RCC->CR : \
								 ((((__FLAG__) >> 5U) == 2U) ? RCC->BDCR : \
								 ((((__FLAG__) >> 5U) == 3U)? RCC->CSR : \
								 ((((__FLAG__) >> 5U) == 4U)? RCC->RSR :RCC->CIFR))))  & \
...
volatile uint32_t reg_debug;  // volatile debug
...
void PLL_Config480Mhz(void)
{
	...
	{
		MODIFY_REG (PWR->CR3, PWR_SUPPLY_CONFIG_MASK, PWR_CR3_LDOEN);
		// as long as not rdy system is in Run* (limited run mode)
		while (!((PWR->CSR1 & PWR_CSR1_ACTVOSRDY) == PWR_CSR1_ACTVOSRDY)){} // wait for LDO
		MODIFY_REG(PWR->D3CR, PWR_D3CR_VOS, PWR_D3CR_VOS_1); // VOS1 for VOS0 setup
		while(!((PWR->D3CR & PWR_D3CR_VOSRDY) == PWR_D3CR_VOSRDY)){} // wait for VOS1
		SET_BIT(RCC->APB4ENR, RCC_APB4ENR_SYSCFGEN); // EN SYSCFG clk
		SET_BIT(SYSCFG->PWRCR, SYSCFG_PWRCR_ODEN); // EN Overdrive to boost to VOS0
		while(!((PWR->D3CR & PWR_D3CR_VOSRDY) == PWR_D3CR_VOSRDY)){} // wait for VOS0
		// reg_debug = PWR->CR3;
		// reg_debug = PWR->D3CR;

		/*Configure MAIN PLL*/
		CLEAR_BIT(RCC->CR, RCC_CR_PLL1ON | RCC_CR_PLL2ON | RCC_CR_PLL3ON); /*Disable pll*/
		while(RCC->CR & RCC_CR_PLL1RDY) {}
		MODIFY_REG(RCC->PLLCKSELR, RCC_PLLCKSELR_PLLSRC, RCC_PLLSOURCE_HSE);
		__RCC_PLL_CONFIG(RCC_PLLSOURCE_HSE,5,192,2,5,2); // N,M,P,Q,R
